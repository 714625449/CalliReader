...
- **训练任务**：✅ **已完成**（step 100,000/100,000）
- **最新 checkpoint**：`caoshu_step100000.pt`
- **Best checkpoint**：`caoshu_best.pt`（step 91877, loss 0.1400）
- **最终 Validation**：Top-1 **53.83%**，Top-5 **74.91%**
- **磁盘空间**：56.3GB 可用
...
1. **数据量决定上限**：655K 样本 → 29%，974K 样本 → 53.83%（目标 55%+）
...
## 八、2026-05-13 更新：数据集优化方案执行记录

### 8.1 Git 状态
- `backup/pipeline-v1` 分支已 push 到远程（17 个 commit）
- GitHub: `https://github.com/714625449/CalliReader.git`

### 8.2 Day 0 探查结果

| 探查 | 状态 | 结果 |
|------|------|------|
| 1：shufazidian vs CCC/Val 配对对照 | ⚠️ 两次超时（30min+60min） | 待优化重跑 |
| 2：颜色空间检查 | ✅ | **不需要 invert**（训练集 7.4 vs shufazidian 15.2，均为黑底） |
| 3：Original 正楷/草书区分 | ✅ | **放弃长尾集**（无现成标签） |
| 4：整幅书法图来源 | ✅ | `/caoshu/imgs/samples/samples/images/` **1,000 张**整幅书法图 |
| 5：InternLM 显存 | ✅ | 离线/在线均可行（24GB 卡够） |

### 8.3 P0：真迹验证集建设

**验证集 A（字块级）**：
- 划分：shufazidian 493 位作者 → **443 训练 + 50 验证**
- 规模：**1,662 张 / 1,140 字**（符号链接，无复制）
- 路径：`/root/sj-tmp/datasets/Validation_Real/main_block/`

**验证集 B（端到端）**：
- 来源：1,000 张整幅图中随机抽 **100 张**
- YOLO 切字：**3,269 个字块**
- 路径：`/root/sj-tmp/datasets/Validation_Real/main_end2end/crops/`

### 8.4 Track 2：P2b rerank 原型

- 50 张整图跑 Pipeline → 1,914 字 → 存 JSON
- InternVL rerank：1,695/1,706 低置信度字被重排（**88.6% 改变率**）
- **关键发现**：89% 的字置信度 < 0.3（端到端 Pipeline 置信度极低）
- rerank 结果：`/root/sj-tmp/p2b_reranked_results.json`

### 8.5 验证集 A 评估与重大发现

**评估结果**：
```
验证集 A (shufazidian 真迹) 评估结果
Top-1: 1238/1507 = 82.15%
Top-5: 1406/1507 = 93.30%
平均置信度: 0.2817
```

**⚠️ 重大发现：数据泄漏**

经确认，`shufazidian` 的 48K 样本在构建 `CCC_split` 时已被 **resize 至 224×224 并转换为 .jpg**，合并进了训练数据。因此：
- **验证集 A 的 82.15% 结果不可信**（数据泄漏）
- 需要重新构建**完全独立的真迹验证集**
- 或者使用**从未参与训练的整幅书法图**（验证集 B）作为真实基线

### 8.6 YOLO 切字质量检查

可视化 10 张整图（`visual_check/`）：
- 横幅/竖幅大字：**切字框基本准确**
- 印章/落款：**偶有误检**
- 密集竖排：**部分框偏紧**
- 整体评估：**切字本身不是瓶颈**，端到端低置信度主要来自识别模型对整图切出字块的适应性问题

### 8.7 修正后的认知

| 指标 | 之前认为 | 修正后 |
|------|---------|--------|
| 真实基线 | 53.83%（被低估） | **未知**（验证集 A 泄漏，验证集 B 缺人工标注） |
| shufazidian 与训练集关系 | 独立数据源 | **已混入 CCC_split/Training** |
| P1 优先级 | 高（补充真迹） | **需重新评估**（已泄漏，增量有限） |
| 端到端瓶颈 | 可能是切字 | 可能是**整图风格 vs 训练集差异** |

### 8.8 下一步选项

| 选项 | 行动 | 说明 |
|------|------|------|
| **A** | **用验证集 B（100张整图）建真实基线** | 需要人工标注 20-50 张即可估算 |
| **B** | **优化 YOLO 切字参数** | 调整 conf/imgsz/NMS，提升切字质量 |
| **C** | **从 CCC_split 中剔除 shufazidian 数据，重建验证集** | 清理工作量大，需追溯来源 |
| **D** | **直接启动 P1（增量训练），但用 CCC_split/Val 作为验证集** | 回到原验证集，避免泄漏 |
