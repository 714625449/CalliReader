# CaoshuReader OOM 优化方案（修正版）

> 基于 plan0510_1.md 的用户反馈修正。核心改动：修正 bf16+autocast 用法、补上 `no_grad` 包裹冻结模块（最大一刀）、YOLO 改用原生 `imgsz=`、裁字从原图坐标系进行。
...
| 场景 | 当前状况 | OOM 根因 |
|------|---------|---------|
| **单字训练 (`train.py`)** | 图像已固定 resize 到 `448×448` | batch_size/grad_accum 过大；ViT 中间激活被保留供反向传播 |
| **验证集测试 (`test_caoshu.py`)** | 同上 | batch_size 过大；同上 |
| **整图 Pipeline (`pipeline.py`)** | YOLO 直接读取**原图**检测 | 超高分辨率扫描件导致 YOLO 前向 OOM |

---

## 二、训练/测试阶段优化（单字 448×448）

### 2.1 核心原则

1. **冻结模块（ViT / MLP1 / tok_embeddings）必须包 `torch.no_grad()`**
   - `requires_grad=False` 只能阻止参数更新，**不能阻止 autograd 保留中间激活**
   - Resampler 的输入来自 ViT→MLP1，反向传播需要回溯到 Resampler 输入，因此 ViT 全路径激活会被保留
   - 用 `no_grad()` 彻底断开梯度图，**这一步省下的显存远大于 autocast 本身**（ViT 12+ 层激活 >> Resampler 4 层）

2. **bf16 不需要 `GradScaler`**
   - `GradScaler` 是为了解决 **fp16 梯度下溢**（指数位只有 5 bit）
   - bf16 指数位和 fp32 一样宽（8 bit），不存在下溢问题，**不需要 scaler**

3. **`autocast` 显式指定 `dtype=torch.bfloat16`**
   - `torch.cuda.amp.autocast()` 默认是 fp16，不是 bf16
   - 不要手动 `imgs.to(torch.bfloat16)`，让 autocast 自动管理支持 op 的 dtype 转换

4. **`torch.cuda.empty_cache()` 不降低峰值**
   - 它只是把 PyTorch reserved 显存还给 CUDA driver
   - OOM 发生在前向/反向的**峰值瞬间**，empty_cache 救不了
   - 可以保留作为例行清理，但不要写在方案里当主要手段
    ...
    while step < args.total_steps:
        for imgs, labels in loader:
            ...
            with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                ...
                with torch.no_grad():   # tok_embeddings 也是冻结的
                    with torch.cuda.amp.autocast(dtype=torch.bfloat16):
...
**关键改动点：**
- 删除 `GradScaler` 相关全部代码
- `get_visual_embed()` 和 `tok_embeddings()` 都用 `torch.no_grad()` 包裹
- `autocast` 显式指定 `dtype=torch.bfloat16`
- 输入 `imgs` 不再手动 `.to(torch.bfloat16)`

### 2.3 `test_caoshu.py` 修改
...
测试阶段全部在 `no_grad()` 下，不需要分层包裹，更简洁。

---

## 三、整图 Pipeline 优化（YOLO 检测阶段）

### 3.1 改用 YOLO 原生 `imgsz=`（代替手动 cv2 resize）

Ultralytics 的 `YOLO.predict()` 原生支持 `imgsz=` 参数，内部会做 **letterbox + pad**，保持长宽比，bbox 坐标自动映射回原图尺度。比手动 `cv2.resize`（会拉伸变形）更精确。
...
class YoloVisualizer:
    ...
    def detect_and_visualize(self,
        ...
        检测并在原图上画框
        Args:
            imgsz: YOLO 推理分辨率，内部自动 letterbox（防止OOM + 保长宽比）
        """
...
**关键优势：**
- `imgsz=1344` 表示 YOLO 长边限制为 1344，短边按比例 letterbox
- bbox 输出**已经是原图坐标系**，不需要手动映射
- 从原图裁字 → resize 到 224，**只有一次 resize**，识别精度无损
...
def process_image(self,
    ...
    """
    Args:
        yolo_imgsz: YOLO 推理分辨率（长边限制，内部 letterbox）
    """
    image_path = Path(image_path)
...
> plan0510_1.md 里 "25MB/样本" 拍脑袋估低了。448×448 过 ViT 的激活显存取决于具体 ViT 规格（层数、hidden_dim、patch_size），实测才准。

**实测方法（插入到 test_caoshu.py 开头）：**
...
测出峰值后，按 `peak / batch_size` 得单样本占用，再反推当前 GPU 能撑的最大 batch_size。
...
|-------|---------|
| `GradScaler` | bf16 不需要 |
| `torch.cuda.empty_cache()` 当核心手段 | 不降低峰值，OOM 救不了 |
| `gradient_checkpointing` on ViT | ViT 已包 `no_grad`，不存激活，checkpointing 无用 |
| 手动 `cv2.resize` + 坐标映射 | YOLO 原生 `imgsz=` + letterbox 更干净 |
